...
def negative_assert_symbol(first_name):
    ...
    assert user_response.json()["message"] == "Имя пользователя введено " \
            "некорректно. Имя может содержать только русские или латинские " \
            "буквы, длина должна быть не менее 2 и не более 15 символов"
...
def negative_assert_no_first_name(user_body):
    ...
    assert user_response.status_code == 400
    assert user_response.json()["code"] == 400
    assert user_response.json()["message"] == "Не все необходимые параметры " \
            "были переданы"
...
def test_create_user_16_letter_in_first_name_get_error_response():
    negative_assert_symbol("aasdfghjklqwerty")

def test_create_user_english_letter_in_first_name_get_success_response():
    positive_assert("QWErty")

def test_create_user_russian_letter_in_first_name_get_success_response():
    positive_assert("Роман")

def test_create_user_has_space_in_first_name_get_error_response():
    negative_assert_symbol("asd asd")

def test_create_user_has_special_symbol_in_first_name_get_error_response():
    negative_assert_symbol("~@sdfsdf")

def test_create_user_has_number_in_first_name_get_error_response():
    negative_assert_symbol("12asdasd")
